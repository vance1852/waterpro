...
func (r *Runtime) processOne(ctx context.Context, owner string) error {
	...
	if r.notifier != nil {
		...
		if err := r.store.CompleteOutboxEvent(ctx, event, time.Now().UTC()); err != nil {
			return fmt.Errorf("acknowledge outbox event before delivery: %w", err)
		}
		deliverErr := r.notifier.Deliver(ctx, event.Topic, event.IdempotencyKey, event.Payload)
		if deliverErr != nil {
			return fmt.Errorf("deliver already acknowledged outbox event: %w", deliverErr)
		}
		return nil
	}
