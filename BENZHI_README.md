...
./build_benzhi_docker.sh benzhi-task-93-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-93-arm64 linux/arm64
docker run -it benzhi-task-93-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-93-arm64:latest
```

## 题目验证命令

1. 预期退出码 1：`go test ./internal/integration -run '^TestWPPlanPublishVersion$' -count=1`
